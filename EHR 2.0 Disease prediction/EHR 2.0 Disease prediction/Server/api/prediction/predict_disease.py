...
workout = pd.read_csv(os.path.join(dataset_path, "workout_df.csv")).fillna('')
...
def helper(dis):
    ...
    workouts = workout[workout['disease'] == dis]['workout']

    # print(workouts)

    if workouts.iloc[1] is None:
        workouts = str(workouts.iloc[0]).split('\n')
...
def predict(symptoms):
    ...
    dis_des, rec_precautions, rec_medications, rec_diet, _workouts = helper(predicted_disease)

    my_precautions = []
    for i in rec_precautions[0]:
    ...
    data = {
        ...
        "medications": json.loads(rec_medications[0].replace("'", '"')),
        "my_diet": json.loads(rec_diet[0].replace("'", '"')),
        "workout": _workouts.tolist()
    }
